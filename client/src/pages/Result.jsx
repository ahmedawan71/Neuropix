    ...
    <form onSubmit={handleSubmit} className="flex flex-col min-h-[90vh] justify-center items-center">
      ...
      {!isImageLoaded && (
        <div className="flex w-full max-w-xl bg-neutral-500 text-white text-sm p-0.5 mt-10 rounded-full">
          <input onChange={e => {e.target.value}} value={input}
            type="text"
      ...
      {isImageLoaded && (
        <div className="flex gap-2 flex-wrap justify-center text-white text-sm p-0.5 mt-10 rounded-full">
          <p onClick={()=>{setisImageLoaded(false)}} className="bg-transparent border border-zinc-900 text-black px-8 rounded-full py-3 cursor-pointer">
            Generate Another
          </p>
          <a
            href={image}
